---
title: "Espécies brasileiras ameaçadas de extinção <span style='font-size: medium;color: gray'>v. 1.0.0</style>"
...
```
Autor: Carlos Eduardo de Siqueira - [carlossiqueira@ima.sc.gov.br](mailto:carlossiqueira@ima.sc.gov.br?subject=App Espécies brasileiras ameaçadas de extinção)
...
ameacadasPR <- readr::read_rds("listas/amecadasPR.rds")
ameacadasSC <- readr::read_rds("listas/amecadasSC.rds")
ameacadasRS <- readr::read_rds("listas/amecadasRS.rds")
                     ...
                     fluidPage(
                       ...
                       fluidRow(
                         column(
                           ...
                           DT::dataTableOutput("dados_recebido"),
                         )
                       )
                     )),
            tabPanel(
...
# Se *nomes* está vazio retorna uma data.frame vazio, se não, executa 
# a função `get.taxa` do pacote flora.
# Uma serie de verificações são executadas antes de passar os nomes para
# `get.taxa`.

# De acordo com o código intern. de nomenclatura botânica, os nomes científicos de 
# espécies devem ser escrito no formato binomial, mas os nomes 
# podem conter as abreviações aff., subsp., cf. sp., ssp. e var.
# Primeiro: verifica-se se as abreviações contém o ponto ao 
# final da abreviação. Se não tem acrescentá-o.
# Segundo: os nomes são filtrados pegando-se as 4 primeiras palavras se contém
# as abreviações var., subsp. ou ssp.
# pega-se as 3 primeiras palavras se contém cf. ou aff.
# e as 2 primeiras se não contém nenhuma das abreviações.
# Assim, é possível filtrar os nomes com ou sem os nomes dos autores.
ifelse(
  length(nomes) == 0,
  nomes_BFG <- data.frame(),
  {
    nomes2 <- nomes |> 
      dplyr::filter(value != "") |> 
...
# para cada nome que não exista em Flora do Brasil 2020. Para se evitar isso, é verificada cada linha da tabela para que pelo menos uma não
# seja vazia. 
# Se há linhas sem NA os nomes colados são comparados com os nomes 
# das listas de espécies ameaçadas do sul do Brasil.
ifelse(
  TRUE %in% !is.na(nomes_BFG[1:length(nomes), 2]),
  {
...
observeEvent(input$planilha, {
  ...
  nomes <- arquivo_recebido()  |>
    ...
    dplyr::mutate(value = dplyr::if_else(
      stringr::str_detect(.data[[coluna_selecionada]], "\\svar\\.?\\b|\\ssubsp\\.?\\b|\\bssp\\.?\\b"),
      stringr::word(.data[[coluna_selecionada]], 1, 4),
      dplyr::if_else(
        stringr::str_detect(.data[[coluna_selecionada]], "\\scf\\.?\\b|\\saff\\.?\\b"),
        stringr::word(.data[[coluna_selecionada]], 1, 3),
        stringr::word(.data[[coluna_selecionada]], 1, 2)
      )
    )) |>
    dplyr::distinct(value)
  ...
  output$dados_recebido <- DT::renderDataTable(server = FALSE, DT::datatable({
